 ...
 function fetchData(id) {
  return Promise.all([getPackage(id), fetchChannels()])
    .then(([packageData, allChannels]) => {
      const packageChannelIds = new Set(packageData.channels.map(c => c.channelId));
      const filteredChannels = allChannels.filter(ch => packageChannelIds.has(ch.id));
...
const closeBtn = modal.querySelector('.modal-btn');
  ...
  document.querySelectorAll('.channels-item[data-package] .channels_link.trigger').forEach(btn => {
    btn.addEventListener('click', e => {
      e.preventDefault();
